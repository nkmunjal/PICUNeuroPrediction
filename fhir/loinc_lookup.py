...
model_data_points = ["pH", "PCO2", "PaO2", "IonCalcium", "Sodium", "Potassium", "BUN",
    ...
    "Age", "GCSMotor", "GCSEye", "GCSVerbal", "GCSTotal", "LOC",
...
model_generator = {
        "pH": partial(np.random.normal,loc=7.4,scale=.05),
        "PCO2": partial(np.random.normal,loc=40,scale=5),
        "PaO2": partial(np.random.normal,loc=90,scale=20),
        "IonCalcium": partial(np.random.normal,loc=1.1,scale=0.2),
        "Sodium": partial(np.random.normal,loc=140,scale=5),
        "Potassium": partial(np.random.normal,loc=4,scale=0.5),
        "BUN": partial(np.random.normal,loc=20,scale=5),
        "Creatinine": partial(np.random.normal,loc=1,scale=0.2),
        "Glucose": partial(np.random.normal,loc=150,scale=20),
        "CO2": partial(np.random.normal,loc=24,scale=3),
        "TotalCalcium": partial(np.random.normal,loc=9,scale=1),
        "WBC": partial(np.random.normal,loc=9,scale=2),
        "Hemoglobin": partial(np.random.normal,loc=13,scale=3),
        "Platelets": partial(np.random.normal,loc=400,scale=150),
        "PT": partial(np.random.normal,loc=18,scale=5),
        "PTT": partial(np.random.normal,loc=40,scale=5),
        "INR": partial(np.random.normal,loc=1.3,scale=.2),
        "Temp": partial(np.random.normal,loc=37,scale=1),
        "RespRate": partial(np.random.normal,loc=18,scale=5),
        "HeartRate": partial(np.random.normal,loc=90,scale=20),
        "SBP": partial(np.random.normal,loc=110,scale=20),
        "DBP": partial(np.random.normal,loc=80,scale=20),
        "Age": partial(np.random.normal,loc=3000,scale=800),
        "GCSMotor": partial(random.randint,1,6),
        "GCSEye": partial(random.randint,1,4),
        "GCSVerbal": partial(random.randint,1,5),
        "GCSTotal": partial(random.randint,3,15),
        "GCSIntub": partial(random.choice,model_choices["GCSIntub"]),
        "LOC": partial(random.choice,model_choices["LOC"]),
        "RightPupilResponse": partial(random.choice,model_choices["RightPupilResponse"]),
        "LeftPupilResponse": partial(random.choice,model_choices["LeftPupilResponse"]),
        "Hypothermia": partial(random.choice,model_choices["Hypothermia"]),
        "Sex": partial(random.choice,model_choices["Sex"]),
        "Location": partial(random.choice,model_choices["Location"])
        }
